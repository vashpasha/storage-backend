...
@router.get("/")
async def workers_list(position: Optional[int] = None, status: Optional[bool]= None, db=Depends(connect_to_database)):
    query = "SELECT * FROM workers"
    
    if status is not None and position:
...
async def worker_detail(id: int, db=Depends(connect_to_database)):
    try: 
        info = await db.fetch("SELECT loaders.number, start_time, end_time, storage.address FROM working JOIN loaders ON working.loader=loaders.id JOIN storages ON working.storage=storage.id WHERE loader=$1", id)
        query = "SELECT * FROM workers WHERE id=$1"
        result = await db.fetchrow(query, id)
        if result:
            return {"worker": result, "works": info}
        else:
...
async def update_worker(id:int, item: WorkerParams, db=Depends(connect_to_database)):
    try:
        previous = await db.fetchrow("SELECT first_name, last_name, position, phone FROM workers WHERE id=$1", id)
        query = """
            UPDATE workers
            SET first_name=$1, last_name=$2, position=$3, phone=$4
            WHERE id=$5
        """
        values = (
